  ...
  return (
    <div className="min-h-screen pb-24 pt-6 px-4 bg-background">
      ...
      >
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-3xl font-black text-foreground mt-1">
              {totalExpected}
              <span className="text-base font-medium text-muted-foreground">h / 168h</span>
            </p>
          </div>
          <div
            className="text-right px-3 py-2"
            style={{ background: '#E3F2FD', border: '3px solid #000000', boxShadow: '3px 3px 0px #000000', borderRadius: 10 }}
          >
        ...
        {/* Stacked bar */}
        <div
          className="h-4 overflow-hidden flex gap-0.5 mb-3"
          style={{ border: '3px solid #000000', borderRadius: 8 }}
        >
          {bubbles.map(b => (
            <div
              key={b.id}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Progress bar */}
                  <div
                    className="relative h-2.5 overflow-hidden mb-1.5"
                    style={{ background: lightBg, border: '2px solid #000000', borderRadius: 6 }}
                  >
                    <div
                      className="absolute left-0 top-0 h-full transition-all duration-700"
                      style={{
                        width: `${Math.min((bubble.actualWeeklyHours / bubble.expectedWeeklyHours) * 100, 100)}%`,
                        background: isOver ? '#FF5252' : color,
                      }}
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      {isOver ? (
                        <>
                          <TrendingUp size={10} color="#FF5252" strokeWidth={2.5} />
                          <span className="text-[10px] font-bold" style={{ color: '#FF5252' }}>+{(bubble.actualWeeklyHours - bubble.expectedWeeklyHours).toFixed(1)}h over</span>
                        </>
                      ) : isMissing ? (
                        <>
                          <TrendingDown size={10} color="#FF9800" strokeWidth={2.5} />
                          <span className="text-[10px] font-bold" style={{ color: '#FF9800' }}>{(bubble.expectedWeeklyHours - bubble.actualWeeklyHours).toFixed(1)}h behind</span>
                        </>
                      ) : (
                        <>
                          <Target size={10} color={color} strokeWidth={2.5} />
                          <span className="text-[10px] font-bold" style={{ color }}>On track</span>
                        </>
                      )}
                    </div>
                  </div>
