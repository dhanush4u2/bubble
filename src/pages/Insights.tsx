...
export default function InsightsPage() {
  ...
  const pieData = defaultBubbles.map(b => ({
    ...
    lightBg: getCategoryLightBg(b.category),
  }));
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          ...
          <div
            ...
            style={{ background: bg, border: '3px solid #000000', boxShadow: '3px 3px 0px #000000', borderRadius: 12 }}
          >
            <p className="text-[9px] font-black text-muted-foreground uppercase tracking-wider mb-1">{label}</p>
            <p className="text-lg font-black" style={{ color }}>{value}</p>
            <div className="flex justify-center mt-1">
              {trend === "up" ? <TrendingUp size={12} color="#4CAF50" strokeWidth={2.5} /> :
               trend === "down" ? <TrendingDown size={12} color="#FF9800" strokeWidth={2.5} /> :
               <Minus size={12} color="#777" strokeWidth={2.5} />}
            </div>
          </div>
        ))}
      </div>
            ...
            {pieData.map(item => (
              <div key={item.name} className="flex items-center gap-2">
                <div
                  ...
                  style={{ background: item.color, border: '1.5px solid #000000' }}
                />
            ...
            return (
              <div key={bubble.id}>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-xs font-black" style={{ color: isOver ? '#FF5252' : color }}>
      ...
      <div className="mb-5">
        ...
        <div className="space-y-3">
          {aiInsights.map(({ Icon, text, color, bg }, i) => (
            <div
              key={i}
              className="flex gap-3 items-start p-4 animate-fade-up"
              style={{
                background: bg,
                border: '3px solid #000000',
